...
@section('breadcrumb', 'Nouvelle candidature')
@section('content')
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="poste" name="poste"
                           value="{{ old('poste') }}"
                           placeholder="Ex : Développeur Laravel, Chef de projet..."
                        ...
                        <div class="relative">
                            <select id="priorite" name="priorite"
                                ...
                                @foreach($priorites as $val => $label)
                                    <option value="{{ $val }}" {{ old('priorite', 'normale') === $val ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="4"
                              placeholder="Informations sur l'entreprise, contact, impression générale..."
                              class="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-medium text-slate-800 placeholder-slate-400 focus:bg-white focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all outline-none leading-relaxed @error('notes') !border-rose-500 ring-rose-500/10 @enderror">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="text-rose-600 text-[11px] font-semibold mt-1.5 pl-0.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 tracking-wide uppercase mb-1.5 pl-0.5" for="fichier">
                        Document joint <span class="text-slate-400 font-normal">(CV, lettre de motivation — PDF/DOC max 5 Mo)</span>
                    </label>
