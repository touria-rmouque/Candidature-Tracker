...
@section('breadcrumb', 'Modifier l\'entretien')
@section('content')
    ...
    <div class="max-w-xl mx-auto antialiased">
        <div class="mb-6">
            ...
            <h1 class="text-2xl font-bold tracking-tight text-slate-900 mt-2 sm:text-3xl">Modifier l'entretien</h1>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 sm:p-8">
            <form method="POST" action="{{ route('entretiens.update', [$candidature, $entretien]) }}">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <select id="type" name="type"
                            ...
                            @foreach($types as $val => $label)
                                <option value="{{ $val }}" {{ old('type', $entretien->type) === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('type')
                    ...
                    <div>
                        ...
                        <select id="resultat" name="resultat"
                            ...
                            @foreach($resultats as $val => $label)
                                <option value="{{ $val }}" {{ old('resultat', $entretien->resultat) === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mt-8 pt-6 border-t border-slate-100">
                    
                    <button type="submit" form="delete-entretien-form" 
                            class="text-center text-sm font-semibold text-rose-600 hover:text-rose-700 px-4 py-2.5 rounded-xl hover:bg-rose-50 transition-colors mr-auto w-full sm:w-auto">
        ...
        <form id="delete-entretien-form" method="POST" action="{{ route('entretiens.destroy', [$candidature, $entretien]) }}"
              onsubmit="return confirm('Supprimer cet entretien définitivement ?')" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
